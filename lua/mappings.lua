...
-- Buffer motions
map("n", "<M-f>", "w", { desc = "Go forward one word" })
map("n", "<M-b>", "b", { desc = "Go backward one word" })
map("i", "<M-f>", "<C-o>w", { desc = "Go forward one word" })
map("i", "<M-b>", "<C-o>b", { desc = "Go backward one word" })
...
cmp.setup {
  mapping = {
    -- Tab to navigate completion suggestions
    ["<Tab>"] = cmp.mapping(function(fallback)
      if cmp.visible() then
        cmp.select_next_item()
      else
        fallback() -- If no completion menu, fall back to normal Tab behavior
      end
    ...
    ["<S-Tab>"] = cmp.mapping(function(fallback)
      if cmp.visible() then
        ...
        fallback() -- If no completion menu, fall back to normal Shift + Tab behavior
      end
    end, { "i", "s" }),
  },
}
